...
from core.queue import dequeue, move_to_dead
from core.job import JobStatus
from core.retry import schedule_retry
from core.metrics import incr, record_timing
from tasks.example import print_message, unstable_task
    ...
    while not SHUTDOWN:
        ...
        try:
            ...
            if retries <= 3:
                ...
                print(f"⚠️ Job {job_id} failed, retry {retries}/3 scheduled")
            else:
                incr("jobs_dead")
                move_to_dead(job_id)
                print(f"☠️ Job {job_id} moved to dead-letter queue")
