...
from core.job import Job, Priority
...
def enqueue_immediate_jobs():
    ...
    for job in jobs:
        ...
        enqueue(job)
        print(f"📤 Enqueued immediate {job.priority.value} job {job.id}")
...
def enqueue_delayed_job(delay_seconds: int = 10):
    ...
    print(f"⏳ Scheduled job {job.id} to run in {delay_seconds}s")
...
def register_cron_job():
    ...
    print("🕒 Cron job registered: cron-job-1 (every minute)")


# ---------- ENTRY POINT ----------
if __name__ == "__main__":
    print("🚀 ForgeQueue Phase 4 Main")
